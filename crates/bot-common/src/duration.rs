...
impl Duration {
    ...
    pub fn as_digital(&self) -> String {
        ...
        let p = partition(std::time::Duration::from_secs(self.0));
    ...
    fn from_str(mut s: &str) -> Result<Self, Self::Err> {
        ...
        while !s.is_empty() {
            match s.find(|c: char| !c.is_numeric()) {
                ...
                Some(i) => {
                    return Err(FromStrError::BadSuffix(s[i..].into()));
                }
                _ => return Err(FromStrError::BadEnd),
            }
        }
...
#[inline(always)]
pub fn partition(duration: std::time::Duration) -> DurationParts {
