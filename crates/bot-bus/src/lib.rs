...
impl Global {
    ...
    pub fn song_progress(song: Option<&Song>) -> Self {
        let song = match song {
            Some(song) => song,
            None => {
        ...
        Global::SongProgress {
            ...
            duration: song.item().duration().as_secs(),
        }
    }

    /// Construct a message that the given song is running.
    pub fn song(song: Option<&Song>) -> Result<Self> {
        let song = match song {
            Some(song) => song,
            None => {
        ...
        Ok(Global::SongCurrent {
            ...
            user: song.item().user().cloned(),
            is_playing: song.state() == State::Playing,
            elapsed: song.elapsed().as_secs(),
            duration: song.item().duration().as_secs(),
        })
    }
