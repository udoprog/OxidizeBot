...
impl Emotes {
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            inner: Arc::new(Inner {
                ...
                streamer,
                ffz: FrankerFaceZ::new(user_agent)?,
                bttv: BetterTTV::new(user_agent)?,
                tduva: Tduva::new(user_agent)?,
    ...
    async fn room_emotes_from_ffz(&self, user: &api::User) -> Result<EmoteByCode> {
    ...
    async fn bttv_bot_badge(&self, user: &api::User, name: &str) -> Result<Option<Badge>> {
        let channel = self
            .inner
            .cache
            .wrap(
    ...
    /// Construct a set of room emotes from bttv.
    async fn room_emotes_from_bttv(&self, user: &api::User) -> Result<EmoteByCode> {
        let channel = self
            .inner
            .cache
            .wrap(
    ...
    async fn emote_sets_from_twitch(&self, emote_sets: &str) -> Result<EmoteByCode> {
    ...
    /// Construct a set of room emotes from ffz.
    async fn emote_sets_from_bttv(&self) -> Result<EmoteByCode> {
        let emotes = self.inner.bttv.emotes().await?;
        self.convert_emotes_from_bttv(emotes.emotes, emotes.url_template)
    }

    /// Get all room emotes.
    async fn room_emotes(&self, user: &api::User) -> Result<Arc<EmoteByCode>> {
        self.inner
            .cache
            .wrap(
                Key::RoomEmotes {
    ...
    /// Get all user emotes.
    fn message_emotes_twitch(&self, tags: &irc::Tags, message: &str) -> Result<EmoteByCode> {
    ...
    /// Get all user emotes.
    async fn global_emotes(&self) -> Result<Arc<EmoteByCode>> {
        self.inner
            .cache
    ...
    async fn tduva_chat_badges(&self, name: &str) -> Result<SmallVec<[Badge; INLINED_BADGES]>> {
        ...
        if let Some(d) = &*self.inner.tduva_data.read().await {
            ...
            if let storage::State::Fresh(..) = entry {
                for b in &d.chatty {
