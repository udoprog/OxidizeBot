...
impl ConnectPlayer {
    ...
    /// is unspecified.
    pub(super) async fn play(&self, id: Option<SpotifyId>, elapsed: Option<std::time::Duration>) {
    ...
    pub(super) async fn set_scaled_volume(&self, scaled_volume: u32) {
        ...
        self.volume(crate::ModifyVolume::Set(update)).await;
    }
...
impl ConnectStream {
    pub(crate) async fn recv(&mut self) -> crate::IntegrationEvent {
        loop {
            if let Some(event) = self.config_rx.recv().await {
                return match event {
                    ConfigurationEvent::DeviceChanged => crate::IntegrationEvent::DeviceChanged,
                };
            }
        }
    }
...
impl ConnectDevice {
    ...
    pub(super) async fn sync_device(&self, device: Option<Device>) -> Result<()> {
    ...
    pub(super) async fn list_devices(&self) -> Result<Vec<Device>> {
