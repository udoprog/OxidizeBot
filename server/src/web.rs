...
/// Setup a simple server to receive the token.
fn receive_token(addr: &SocketAddr) -> Result<ReceivedToken, failure::Error> {
    use hyper::service::service_fn_ok;
    use hyper::{Body, Request, Response, Server, StatusCode};
    use std::sync::mpsc;

    let (token_tx, token_rx) = mpsc::sync_channel::<ReceivedToken>(1);
        ...
        service_fn_ok(move |req: Request<Body>| {
            ...
            for (key, value) in utils::query_pairs(query) {
                ...
                match (key.as_ref(), value) {
                    ...
                    (key, value) => {
                        log::info!("unhandled query parameter: {} = {}", key, value);
                    }
                }
            }
            ...
            tx.send(ReceivedToken { state, code })
                .expect("closed channel");
            Response::new(Body::from("Token received, feel free to close the window."))
        })
    };
    ...
    let server_thread = std::thread::spawn::<_, Result<(), failure::Error>>(move || {
        let mut rt = tokio::runtime::Runtime::new()?;
        rt.block_on(server)?;
        Ok(())
    });

    let token = token_rx.recv().expect("closed channel");
    let _ = tx.send(());
    server_thread.join().expect("thread panicked")?;
    Ok(token)
}
*/
